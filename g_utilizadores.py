...
def apagar_utilizador():
    ...
    if selecionado:
        ...
        # Check if the utilizador is associated with a curso
        cursor = conexao.cursor()
        cursor.execute("SELECT aluno_id FROM q_alunos_cursos WHERE aluno_id = %s", (utilizador_id,))
        result = cursor.fetchone()
        cursor.close()  # Close the cursor object

        if result:
            messagebox.showinfo("Aviso", "O utilizador está associado a um curso e não pode ser excluído diretamente.")
            return
        ...
        if messagebox.askyesno("Confirmação", f"Deseja apagar o utilizador com ID {utilizador_id}?"):
            try:
                ...
                # Em caso de erro, imprimir a mensagem
...
def criar_utilizador():
    def salvar_utilizador():
        ...
        if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            # Mostrar erro de entrada
            messagebox.showwarning("Aviso", "Invalid email address.")
            return
        contacto = entry_contacto.get()
        if not contacto.isdigit() or len(contacto) != 9:
            # Mostrar erro de entrada
            messagebox.showwarning(text="Invalid phone number.")
            return
        morada = entry_morada.get()
        localidade = entry_localidade.get()
        nascimento = data_nascimento.get()
        senha = entry_senha.get()
        perfil = entry_perfil.get()

        if perfil == "1":
            messagebox.showinfo("Perfil", "O utilizador irá ficar registado como Aluno")
        elif perfil == "2":
            messagebox.showinfo("Perfil", "O utilizador irá ficar registado como Professor")
        elif perfil == "3":
            messagebox.showinfo("Perfil", "O utilizador irá ficar registado como Administrador")
        else:
            messagebox.showerror("Erro", "Não existe nenhum utilizador com esse ID")
    ...
    data_nascimento = DateEntry(janela_utilizador, width=10, background='darkblue', foreground='white', borderwidth=2,
                                   justify='center', date_pattern='yyyy-mm-dd')
    ...
    data_nascimento.pack(padx=5, pady=5)
...
def alterar_utilizador():
    ...
    if selecionado:
        ...
        data_nascimento = DateEntry(janela_alterar, width=12, background='darkblue', foreground='white',
                                        borderwidth=2,
                                        justify='center', date_pattern='yyyy-mm-dd')

        data_nascimento.insert(0, utilizador[6])
        data_nascimento.grid(row=5, column=1)
...
def criar_interface(content_frame):
    ...
    table.column("Contacto", width=80)
    table.column("Morada", width=100)
    table.column("Localidade", width=100)
    table.column("Nascimento", width=70)
    table.column("Password", width=70)
    table.column("Perfil", width=50)
